...
Relationship_vs_Academic <- ggplot(mydata, mapping = aes(x = teacher_student_relationship, y = academic_performance)) + geom_jitter(width = 0.2, alpha = 0.4) + geom_smooth(method = "lm", se = FALSE, color = "blue") + labs(title = "Teacher/Student Relationship vs Academic Performance", x = "Teacher/Student Relationship", y = "Academic Performance")
...
ggplot(mydata, mapping = aes(x = study_load, y = stress_level, color = social_support)) + geom_point(position = "jitter") + geom_smooth(method = "lm")
